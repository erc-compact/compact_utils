...
def plotPackedBeam(coordinates, angle, axis1, axis2, boresight,
        tiling_meta, core_radius, half_light_radius, half_mass_radius, central_freq, fileName='pack.png', scope=1., show_axis = True,
    ...
    circle_core = Circle(xy=(0,0), radius = core_radius/step, linestyle='--', linewidth=4, fill=False, label='Core', color='r')
    circle_half_light = Circle(xy=(0,0), radius = half_light_radius/step, linestyle='-.', linewidth=4, fill=False, label='Half light', color='b')
    circle_half_mass = Circle(xy=(0,0), radius = half_mass_radius/step, linestyle=':', linewidth=4, fill=False, label='Half mass', color='k')

    axis.add_artist(circle_core)
    axis.add_artist(circle_half_light)
    axis.add_artist(circle_half_mass)
    gbt_dish_diameter = 100
    gbt_beam_size = get_fwhm_of_telescope(gbt_dish_diameter, central_freq)
    gbt_beam_size_radius = gbt_beam_size/2
    gbt_beam = Circle(xy=(0,0), radius = gbt_beam_size_radius/step, linestyle='dotted', linewidth=4, fill=False, label='GBT beam', color='g')
    axis.add_artist(gbt_beam)
    axis.legend(handles=[circle_core, circle_half_light, circle_half_mass, gbt_beam], prop={'size': 40})
...
def get_fwhm_of_telescope(dish_diameter, freq):
    """
    dish_diameter: in meter
    freq: in Hz
    """
    c = 299792458.0
    try:
        ...
        wavelength = c/freq[0]
    beam_size = math.degrees(1.22*wavelength/dish_diameter)
    return beam_size


#def plot_core_half_light_radius(boresight, fileName, core, half_light_radius, HD==True):
#
#    step = 1/10000000000.
#    wcs_properties = wcs.WCS(naxis=2)
#    wcs_properties.wcs.crpix = [0, 0]
...
#    axis.add_artist(circle_core)
#    axis.add_artist(circle_half_light)
#    axis.legend(handles=[circle_core, circle_half_light], prop={'size': 40})
#    plt.xlabel('Right Ascension')
#    plt.ylabel('Declination')
#    plt.savefig(fileName, dpi=thisDpi)
#    plt.close()
